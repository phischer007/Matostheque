...
####################################################################################
# Retrieve materials based on category (TYPE_CHOICES and LAB_SUPPLY_TYPE_CHOICES)  #
####################################################################################
@api_view(['GET'])
def get_materials_by_category(request, category):
    """
    Retrieve materials filtered by their category (type or lab_supply_type).
    """
    materials = Materials.objects.filter(
        F(type=category) | F(lab_supply_type=category)
    )
...
# Check availability of materials in a specific category
@api_view(['GET'])
def check_material_availability(request, category):
    """
    Retrieve materials and their availability status for a given category.
    """
    materials = Materials.objects.filter(
        F(type=category) | F(lab_supply_type=category)
    ).values(
        'material_title', 'availability', 'loan_duration'
    )
